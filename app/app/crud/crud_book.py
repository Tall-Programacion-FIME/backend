...
def create_book(
    db: Session, es: Elasticsearch, book: schemas.BookCreate, user_id: int
) -> schemas.Book:
...
def update_book(
    db: Session, es: Elasticsearch, book: schemas.BookUpdate, book_id: int
) -> schemas.Book:
    db_book: models.Book = (
    ...
    es.update(
        index="books",
        id=es_book_id,
        body={"doc": {"name": book.name, "author": book.author}},
    )
    return db_book


def delete_book(
    background_tasks: BackgroundTasks, db: Session, es: Elasticsearch, book_id: int
):
    db_book: models.Book = (
        db.query(models.Book).filter(models.Book.id == book_id).first()
    )
...
def get_book_elastic_id(es: Elasticsearch, book_id: int) -> str:
    es_book = es.search(
        index="books", body={"query": {"match": {"id": book_id}}, "size": 1}
    )
