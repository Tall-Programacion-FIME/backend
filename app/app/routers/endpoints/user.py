...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    if not utils.is_valid_email_domain(user.email):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Correo invalido.")
    db_user = crud.get_user_by_email(db, email=user.email)
    banned_user = crud.get_banned_user(db, email=user.email)
    if banned_user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Has sido baneado.")
    if db_user:
...
def ban_user(background_tasks: BackgroundTasks, user_id: int, db: Session = Depends(get_db),
             es: Elasticsearch = Depends(get_es), admin: schemas.User = Depends(get_current_admin)):
    user = crud.get_user(db, user_id=user_id)
    crud.ban_user(background_tasks=background_tasks, db=db, es=es, user_email=user.email)
